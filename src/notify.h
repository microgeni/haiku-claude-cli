...
//
//   const auto urls = notify::ExtractUrls(reply);
//   notify::Send(notify::PickPlayfulTitle(elapsed),
//                notify::FirstSentence(reply, 120));
//
// On non-Haiku builds `send()` is a no-op so macOS dev under nix
// stays silent. `ExtractUrls` is also used by /open (REPL slash
