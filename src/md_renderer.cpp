...
// ---------------------------------------------------------------------------
// Table helpers — pure string logic lives in md_text.cpp (unit-tested);
// these thin members forward so the class API stays unchanged.
// ---------------------------------------------------------------------------

bool MdRenderer::IsTableRow(const std::string& line)
{
	return md::IsTableRow(line);
}

bool MdRenderer::IsSeparatorRow(const std::string& line)
{
	return md::IsSeparatorRow(line);
}

std::vector<std::string> MdRenderer::SplitCells(const std::string& line)
{
	return md::SplitCells(line);
}
...
// ---------------------------------------------------------------------------
// StripHtml — moved to md_text.cpp (pure logic, unit-tested). Callers still
// reach it via md::StripHtml declared in md_text.h.
// ---------------------------------------------------------------------------
